...
export default function CityPage() {
  ...
  return <div className="page neighborhood-page city-page">
    ...
    <FocusedWorkspace
      ...
      runs={(history.data ?? []).map((item) => ({ id: item.id, label: item.scenario_name || item.scientific_status || item.id.slice(0, 8), meta: `${formatDate(item.created_at, i18n.language)} · ${item.id.slice(0, 8)}` }))}
