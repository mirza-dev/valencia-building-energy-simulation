...
test('builder validates, previews exact geometry, commits, and exports', async ({ page }) => {
  ...
  await page.getByRole('link', { name: /Kanıt ve dosyalar|Evidence & files/ }).click()
...
test('Phase E0 inspects the materialized OpenStudio graph and cross-highlights constructions', async ({ page, request }) => {
  ...
  await page.getByTitle('TR / EN').click()
...
test('historical system screens remain directly reachable without entering navigation', async ({ page }) => {
  ...
  await page.getByTitle('TR / EN').click()
  await expect(page.locator('html')).toHaveAttribute('lang', 'tr')
  await expect(page.getByRole('heading', { name: 'Veri sağlığı' })).toBeVisible()
  await page.getByTitle('TR / EN').click()
  await expect(page.locator('html')).toHaveAttribute('lang', 'en')
  await page.getByRole('link', { name: /Varyantları karşılaştır|Compare variants/ }).click()
  await expect(page.getByRole('heading', { name: /Aktif bina karşılaştırması|Active-building comparison/ })).toBeVisible()
...
test('building selection becomes the persistent scope for the visible workflow', async ({ page }) => {
  await page.goto('/#/builder')
  await page.getByRole('button', { name: /Building|Bina/ }).click()
  const search = page.getByPlaceholder(/Global refparcela ara|Search refparcela/)
  await search.fill('4252701YJ2745A')
  await expect(page.getByRole('button', { name: /4252701YJ2745A/ })).toBeVisible()
  await page.getByRole('button', { name: /4252701YJ2745A/ }).click()
  await expect(page.locator('.active-building-card')).toContainText('4252701YJ2745A')
  await page.getByRole('link', { name: /Simülasyon|Simulation/ }).click()
  await expect(page.locator('.topbar-center')).toContainText('4252701YJ2745A')
  await page.getByRole('link', { name: /Varyantları karşılaştır|Compare variants/ }).click()
  await expect(page.locator('.topbar-center')).toContainText('4252701YJ2745A')
})
